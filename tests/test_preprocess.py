...
def test_lr_preprocess(init_params, cleanup_output_dir):
    ...
    df = pd.read_csv(os.path.join(init_params['output_dir'], f"{init_params['output_prefix']}.features"), sep="\t")
    expected_df = pd.read_csv("tests/expected_results/preprocess/test.features", sep="\t")
