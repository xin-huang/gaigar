...
def test_FeatureVectorsPreprocessor(data_params, feature_params):
    generator = GenomicDataGenerator(**data_params)
    preprocessor = FeatureVectorsPreprocessor(**feature_params)
    ...
    num_features += len(features.keys())-3 # Remove `Ref distances`, `Tgt distances`, `Spectra`
    ...
    assert len(items[0].keys()) == num_features+4, "The number of estimated and expected features do not match."
