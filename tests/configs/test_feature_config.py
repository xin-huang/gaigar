...
def test_valid_sstar_full():
    ...
    m = FeatureConfig.model_validate(cfg)
    assert m.root["sstar"]["match bonus"] == 5000
...
def test_dist_unknown_key_raises():
    with pytest.raises(ValidationError) as ei:
        FeatureConfig.model_validate({"ref_dist": {"median": True}})
...
def test_all_conflicts_with_others_true():
    with pytest.raises(ValidationError) as ei:
        FeatureConfig.model_validate({"ref_dist": {"all": True, "mean": True}})
    assert "'all' conflicts" in str(ei.value)
...
def test_sstar_value_must_be_int():
    with pytest.raises(ValidationError) as ei:
        FeatureConfig.model_validate({"sstar": {"match bonus": 5.5}})
    msg = str(ei.value)
    assert "valid integer" in msg and "fractional part" in msg

    with pytest.raises(ValidationError) as ei:
        FeatureConfig.model_validate({"sstar": {"match bonus": True}})
    assert "must be int" in str(ei.value)


def test_sstar_max_mismatch_negative_raises():
    with pytest.raises(ValidationError) as ei:
        FeatureConfig.model_validate({"sstar": {"max mismatch": -1}})
    assert "'max mismatch' must be >= 0" in str(ei.value)


def test_sstar_mismatch_penalty_positive_raises():
    with pytest.raises(ValidationError) as ei:
        FeatureConfig.model_validate({"sstar": {"mismatch penalty": 1}})
    assert "'mismatch penalty' should be <= 0" in str(ei.value)
