...
def _valid_kwargs() -> dict:
    """Return a minimal set of valid kwargs for SimulationConfig."""
    return {
        "nrep": 10,
        "nref": 20,
        "ntgt": 20,
...
def test_simulation_config_valid():
    """A fully valid configuration should be accepted."""
...
def test_simulation_config_positive_int_fields_must_be_gt_zero(
    ...
    """Integer fields with gt=0 should reject non-positive values."""
...
def test_simulation_config_mut_rate_must_be_strictly_positive(
    ...
    """mut_rate has gt=0.0 and must be strictly positive."""
    kwargs = _valid_kwargs()
    kwargs[field] = bad_value
...
def test_simulation_config_rec_rate_must_be_ge_zero(field: str, bad_value: float):
    """rec_rate has ge=0.0 and must not be negative."""
    kwargs = _valid_kwargs()
    kwargs[field] = bad_value
...
def test_simulation_config_props_in_closed_interval(field: str, value: float):
    """intro_prop and non_intro_prop in [0, 1] should be accepted."""
    kwargs = _valid_kwargs()
    kwargs[field] = value
...
def test_simulation_config_props_out_of_range(field: str, value: float):
    """intro_prop and non_intro_prop outside [0, 1] should raise."""
    kwargs = _valid_kwargs()
    kwargs[field] = value
...
def test_simulation_config_output_dir_normalization():
    """output_dir should be normalized via the validator."""
    kwargs = _valid_kwargs()
...
def test_simulation_config_default_flags():
    """Boolean flags should have the expected defaults."""
    cfg = SimulationConfig(**_valid_kwargs())
