...
def test_preprocess_config_valid():
    """A fully valid configuration should be accepted and normalized."""
...
def test_preprocess_config_positive_int_fields_must_be_gt_zero(
    ...
    """Integer fields with gt=0 must reject non-positive values."""
...
def test_preprocess_config_output_dir_normalization():
    """output_dir should be expanded and resolved to an absolute path."""
    kwargs = _valid_kwargs()
...
def test_preprocess_config_accepts_anc_allele_file():
    """anc_allele_file should be optional and converted to Path when provided."""
    kwargs = _valid_kwargs()
...
def test_preprocess_config_missing_required_field_raises():
    """Missing a required field like vcf_file should raise a ValidationError."""
    kwargs = _valid_kwargs()
    kwargs.pop("vcf_file")
