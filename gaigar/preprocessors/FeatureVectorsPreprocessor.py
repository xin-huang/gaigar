...
class FeatureVectorsPreprocessor(DataPreprocessor):
    ...
    def __init__(self, ref_ind_file: str, tgt_ind_file: str, feature_config: str):
        ...
        try:
            ...
            raise FileNotFoundError(f"Feature configuration file {feature_config} not found.")
    ...
    def run(self, chr_name: str, start: int, end: int, ploidy: int, is_phased: bool,
            ref_gts: np.ndarray, tgt_gts: np.ndarray, pos: np.ndarray) -> list[dict[str, Any]]:
        """
    ...
    def _fmt_res(self, res: dict[str, Any], ploidy: int, is_phased: bool,
                 samples: dict[list[str]], features: dict[str, Any]) -> list[dict[str, Any]]:
        """
        ...
        """
        if is_phased: num_samples = len(samples['Tgt']) * ploidy
        else: num_samples = len(samples['Tgt'])

        base_dict = {'Chromosome': res['chr_name'], 'Start': res['start'], 'End': res['end']}
        sample_dicts = [base_dict.copy() for _ in range(num_samples)]
        for i, sample_dict in enumerate(sample_dicts):
            sample = f'{samples["Tgt"][int(i/ploidy)]}_{i%ploidy+1}' if is_phased else samples["Tgt"][i]
            ...
            for pop in ['Ref', 'Tgt']:
                if (f'{pop} distances' in features) and (features[f'{pop} distances']):
                    sample_dict.update(self._fmt_dist_res(
                        row=res, 
                        idx=i, 
                        is_phased=is_phased, 
                        ploidy=ploidy, 
    ...
    def _fmt_dist_res(self, row: dict[str, Any], idx: int, is_phased: bool, ploidy: int,
                      samples: dict[list[str]], features: dict[str, Any], pop: str) -> dict[str, float]:
        """
        ...
        for feature in ['Minimum', 'Maximum', 'Mean', 'Median', 'Variance', 'Skew', 'Kurtosis']:
        ...
        if dist_features.get('All', False):
            if is_phased:
                for j in range(len(row[f'All_{pop}_dists'][idx])):
                    ...
                    items[f"{pop}_dist_{sample}_{j%ploidy+1}"] = row[f'All_{pop}_dists'][idx][j]
            else:
                for j in range(len(row[f'All_{pop}_dists'][idx])):
